...
function extractMessageId(msgObj) {
  ...
  // Wrapped result object from safe send: { ok, sender, res }
  if (msgObj.res && typeof msgObj.res === 'object') {
    const nested = extractMessageId(msgObj.res);
    if (nested) return nested;
  }

  // whatsapp-web.js direct shape: id._serialized
  if (msgObj.id && typeof msgObj.id === 'object' && msgObj.id._serialized) {
    return String(msgObj.id._serialized).trim();
  }
