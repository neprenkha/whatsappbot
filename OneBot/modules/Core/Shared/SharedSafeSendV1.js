...
function pickSend(meta, preferCsv) {
  ...
  for (const name of prefer) {
    const svc = services[name] || (meta && typeof meta.getService === 'function' ? meta.getService(name) : null);
    if (!svc) continue;
...
async function safeSend(meta, preferCsv, doSendFn, opts, log) {
  const prefer = preferCsv || 'sendout,outsend,send';
...
async function send(log, senderOrMeta, chatId, payload, opts) {
  ...
  const prefer = o.sendPrefer || 'sendout,outsend,send';
