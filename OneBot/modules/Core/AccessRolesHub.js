...
module.exports.init = async function init(meta) {
  ...
  if (!implFile) {
    meta.log('AccessRolesHub', `module.error id=${meta.id}, Missing implFile in hubConf, hubConfPath=${meta.hubConfPath}`);
  ...
  try {
    ...
    meta.log('AccessRolesHub', `module.error id=${meta.id}, Require failed: ${err.message}, file=${absImpl}`);
    return { onEvent: async () => {}, onMessage: async () => {} };
  }
  ...
  if (!impl || typeof impl.init !== 'function') {
    meta.log('AccessRolesHub', `module.error id=${meta.id}, Impl missing init(), file=${absImpl}`);
    return { onEvent: async () => {}, onMessage: async () => {} };
  }
