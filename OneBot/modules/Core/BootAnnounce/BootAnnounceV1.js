...
module.exports.init = async function init(meta) {
  ...
  const outboxService = String(conf.outboxService || 'outbox').trim();
  ...
  async function announce(triggerType) {
    ...
    const outbox = meta.getService && meta.getService(outboxService);
    if (outbox && typeof outbox.send === 'function') {
      try {
        await outbox.send(controlGroupId, text, { isAuto: 1 });
        meta.log('BootAnnounceV1', `queued via ${outboxService} after send failure`);
        sendFailureLogged = false;
        return;
      } catch (qErr) {
        meta.log('BootAnnounceV1', `outbox queue failed svc=${outboxService} msg=${qErr && qErr.message ? qErr.message : qErr}`);
      }
    }

    // Log failure only once after all retries exhausted
    if (!sendFailureLogged) {
      meta.log('BootAnnounceV1', `send failed after ${maxRetries} attempts via ${sender.name} msg=${lastError && lastError.message ? lastError.message : lastError}`);
      sendFailureLogged = true;
    }
  }
